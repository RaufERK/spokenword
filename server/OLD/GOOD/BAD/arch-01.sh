...
SRC="rtmp://127.0.0.1/live/ITMUG2025"     # внутренняя петля – безопаснее
...
while [ -f "$FLAG" ]; do
  ...
  /usr/bin/ffmpeg -hide_banner -loglevel error -i "$SRC" \
    ...
    -c:v libx264 -preset veryfast -profile:v:0 high -profile:v:1 main -profile:v:2 baseline \
    -f hls -hls_time 4 -hls_flags independent_segments+delete_segments+temp_file \
