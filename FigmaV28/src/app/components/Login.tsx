...
export function Login() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (login && password) {
      ...
      // Устанавливаем дату оплаты (20 дней назад для теста)
      const paymentDate = new Date();
      paymentDate.setDate(paymentDate.getDate() - 20); // 20 дней назад
      localStorage.setItem("userPaymentDate", paymentDate.toISOString());
      
      // Устанавливаем тестовые ссылки конференции для демо
      if (!localStorage.getItem("conferenceYoutubeLink")) {
        localStorage.setItem("conferenceYoutubeLink", "https://youtube.com/live/example");
      }
      if (!localStorage.getItem("conferenceRutubeLink")) {
        localStorage.setItem("conferenceRutubeLink", "https://rutube.ru/live/example");
      }
