...
## 2025-10-01 — Single Profile HLS (360p) - СТАБИЛЬНАЯ ВЕРСИЯ ⭐

### 🎯 Финальное решение:
После тестирования adaptive HLS с 3 профилями (падал через 1-2 минуты) перешли на **ОДИН стабильный профиль 360p**.

### ✅ Что работает:

#### **FFmpeg HLS транскодирование (360p @ 600 Kbps):**
- Один профиль вместо трёх
- БЕЗ B-frames (`-bf 0`) для стабильности
- Baseline profile для максимальной совместимости
- Keyframe каждые 2 секунды
- Плейлист 30 сегментов (60 секунд буфера)
- **СТАБИЛЬНО:** работает без падений

#### **Forward на платформы:**
- YouTube: 720p @ 1500k (copy, оригинал) ✅
- RuTube: 720p @ 1500k (copy, оригинал) ✅
- Icecast: MP3 @ 128k ✅

### 📁 Файлы:
- `nginx/nginx.conf` - убран push и internal server
- `scripts/start-hls-transcoding.sh` - один профиль 360p
- `scripts/stop-hls-transcoding.sh` - корректная остановка

### ⚠️ Известные проблемы:

**FFmpeg HLS не запускается автоматически через exec_publish**
- Причина: возможный конфликт с порядком exec_publish или таймаут
- Временное решение: запуск вручную `npm run hls:start`
- Планируется исправить: переставить exec_publish в конец списка

### 🎬 Workflow:
1. Запустить OBS
2. Выполнить `npm run hls:start` (вручную)
3. Стрим работает стабильно

### 📊 Результаты:
- ✅ Видео на сайте: 360p @ 600k (отлично для текста)
- ✅ YouTube/RuTube: 720p @ 1500k (максимальное качество)
- ✅ Стабильность: не падает
- ✅ Оптимизация для мобильных: низкий битрейт

---

## 2025-10-01 — Adaptive HLS с FFmpeg транскодированием ⭐ (v2 - ИСПРАВЛЕНО)
...
**Проблема:** В первой версии конфигурации была строка `deny play all` в основном RTMP application, что блокировало:
- ❌ Скрипты Icecast (не могли читать поток)
- ❌ FFmpeg (не мог читать для перекодирования)
- ❌ Forward скрипты (YouTube, RuTube)

**Исправление:** Убрана блокировка из основного application, оставлена только в internal (локальном).
