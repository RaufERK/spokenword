...
### Проблема: Стрим не работает на мобильных устройствах
**Причина:** Недостающие CORS заголовки или неправильные настройки HLS.js для мобильных
**Решение:** 
```bash
# На сервере (ssh amster)
# 1. Исправить конфигурацию nginx с дополнительными CORS заголовками
# 2. Добавить обработку OPTIONS запросов
# 3. Обновить HlsPlayer с мобильными настройками

sudo nginx -t && sudo systemctl reload nginx
```
Или используйте скрипт: `./scripts/test-mobile-compatibility.sh`
...
- **iPhone/iPad Safari:** Нативный HLS с оптимизированными настройками
- **Android Chrome:** HLS.js с мобильными настройками
- **Другие мобильные браузеры:** HLS.js с автоматическим выбором качества

### Особенности мобильной поддержки:
- ✅ Автоматический выбор качества видео
- ✅ Оптимизированные настройки буферизации
- ✅ Поддержка inline воспроизведения (не в полноэкранном режиме)
- ✅ CORS заголовки для всех мобильных браузеров
