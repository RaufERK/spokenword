...
## ⚠️ ВАЖНО: Отдельный репозиторий

Мобильное приложение — это **отдельный проект**, не часть этого репозитория.

| Репозиторий | Назначение | Технологии |
|-------------|------------|------------|
| `spokenword` (этот) | Веб-приложение + API сервер | Next.js SSR, Server Components, Prisma |
| `spokenword-mobile` (новый) | Android-приложение | React + Vite + Capacitor |

**Почему отдельно:**
- Веб использует Server Components — не совместимо со статическим экспортом
- Разные сборки и деплой процессы
- Мобильное приложение — только UI, все данные через API

**Что нужно в ЭТОМ репо:**
- Добавить `/api/health` эндпоинт (для проверки доступности сервера)
- Убедиться что все API работают с CORS для мобильного приложения
...
### Новый репозиторий: `spokenword-mobile`
...
│   │   └── AppInitializer.tsx   # Splash + инициализация
│   ├── pages/
│   │   ├── Home.tsx             # Главная (трансляции)
│   │   ├── Chat.tsx             # Чат
│   │   ├── Login.tsx            # Авторизация
│   │   ├── Register.tsx         # Регистрация
│   │   ├── Profile.tsx          # Профиль
│   │   ├── Archive.tsx          # Архив конференций
│   │   ├── Class.tsx            # Занятия
│   │   ├── PaidContent.tsx      # Платные материалы
│   │   └── Audio.tsx            # Аудио
│   └── styles/
│       └── globals.css          # Tailwind стили
└── dist/                        # Сборка (для Capacitor)
```

### В ЭТОМ репозитории (`spokenword`) нужно добавить:
...
└── app/
    └── api/
        └── health/
            └── route.ts         # Health check эндпоинт ← ДОБАВИТЬ
```
...
### Этап 2: Создание нового репозитория (1 день)

#### 2.1. Создать новый проект

```bash
# Создать папку рядом с веб-проектом
cd ~/Documents/WEB_PROJ/PRODUCTION
mkdir spokenword-mobile
cd spokenword-mobile

# Инициализировать проект с Vite + React + TypeScript
...
#### 2.2. Установить Capacitor и зависимости
...
#### 2.3. Инициализация Capacitor

```bash
npx cap init "SpokenWord" "ru.spokenword.app" --web-dir=dist
npx cap add android
```

#### 2.4. Создать конфигурацию Capacitor
...
#### 2.5. Инициализировать Git

```bash
git init
git remote add origin git@github.com:RaufERK/spokenword-mobile.git
```

#### 2.6. Создать `.gitignore`
...
**Файл:** `src/lib/servers.ts` (в репо `spokenword-mobile`)
...
**Файл:** `src/lib/serverSelector.ts` (в репо `spokenword-mobile`)
...
**Файл:** `src/lib/apiClient.ts` (в репо `spokenword-mobile`)

```typescript
import { selectBestServer, forceServerSwitch, getCurrentServer } from '@/lib/serverSelector'
...
### Этап 4: Создание страниц (2-3 дня)

> Все страницы создаются с нуля в репо `spokenword-mobile`, но можно копировать логику из веб-версии.

#### 4.1. Страницы для мобильного приложения

| Страница | Файл в `spokenword-mobile` | Откуда копировать логику |
|----------|---------------------------|-------------------------|
| Главная | `src/pages/Home.tsx` | `app/page.tsx` |
| Логин | `src/pages/Login.tsx` | `app/login/page.tsx` |
| Регистрация | `src/pages/Register.tsx` | `app/register/page.tsx` |
| Чат | `src/pages/Chat.tsx` | `app/chat/page.tsx` ← почти готов |
| Профиль | `src/pages/Profile.tsx` | `app/profile/ProfileClient.tsx` |
| Архив | `src/pages/Archive.tsx` | `app/conf-arch/page.tsx` |
| Занятия | `src/pages/Class.tsx` | `app/class/page.tsx` |
| Платный контент | `src/pages/PaidContent.tsx` | `app/paid-content/PaidContentClient.tsx` |
| Аудио | `src/pages/Audio.tsx` | `app/audio/page.tsx` |

#### 4.2. Роутинг (React Router)
...
#### 4.3. API запросы — использовать apiClient

**Все fetch заменяются на:**
```typescript
import { apiFetch, apiGet, apiPost } from '@/lib/apiClient'

// Вместо fetch('/api/chat/rooms')
const rooms = await apiGet<Room[]>('/api/chat/rooms')

// Вместо fetch('/api/chat/rooms', { method: 'POST', body: ... })
const result = await apiPost('/api/chat/rooms', { type: 'PRIVATE', participantId })
```

#### 4.4. Обработка авторизации
...
**Файл:** `src/lib/auth.ts` (в репо `spokenword-mobile`)
...
### Этап 5: UI компоненты (1 день)

> Все компоненты в репо `spokenword-mobile`

#### 5.1. Индикатор сервера

**Файл:** `src/components/ServerStatus.tsx`

```typescript
import { useEffect, useState } from 'react'
import { getCurrentServer, getServerInfo, onServerChange } from '@/lib/serverSelector'
...
#### 5.2. Инициализатор приложения (экран загрузки)

**Файл:** `src/components/AppInitializer.tsx`
...
### В репозитории `spokenword` (веб-сервер)

```bash
# Добавить /api/health и задеплоить
npm run deploy
```

### В репозитории `spokenword-mobile` (мобильное приложение)
...
# Запуск на устройстве/эмуляторе с live reload
npx cap run android --livereload --external

# Только копирование файлов (без плагинов)
npx cap copy android
```
...
| Неделя | Репозиторий | Этапы | Результат |
|--------|-------------|-------|-----------|
| **1** | `spokenword` | 1 | `/api/health` эндпоинт + CORS |
| **1** | `spokenword-mobile` | 2-3 | Новый проект + Capacitor + модуль серверов |
| **2** | `spokenword-mobile` | 4-5 | Страницы + UI компоненты |
| **2** | `spokenword-mobile` | 6 | Android-проект (иконки, splash) |
| **3** | `spokenword-mobile` | 7-8 | Тестирование + публикация в Google Play |
